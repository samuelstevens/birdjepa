...
Compared checkpoint param_norm vs logged train/param_norm for sweep 3309878 using scripts/compare_ckpt_param_norms.py. CPU and GPU results match for tisomkvs, so this is not a CPU/GPU restore issue. All runs show checkpoint param_norm consistently lower than log param_norm, with the gap growing with step and far exceeding update_norm. Component norms (encoder/objective/probe) show the same downward bias.
