...
class Supervised(Objective):
    ...
    def __init__(
        self,
        encoder_cfg: transformer.Config,
        n_classes: int,
        n_sources: int = 0,
        source_rank: int = 0,
    ...
    def forward(
        ...
        # Patchify and encode
        x_bnk, grid_bn2 = transformer.patchify(data_bhw, self.encoder_cfg)
...
class LeJEPA(Objective):
    ...
    def __init__(self, encoder_cfg: transformer.Config, cfg: LeJEPAConfig):
        super().__init__()
        self.encoder_cfg = encoder_cfg
        self.cfg = cfg
    ...
    def forward(
        ...
        # Patchify and encode
        x_bnk, grid_bn2 = transformer.patchify(views_flat, self.encoder_cfg)
        out = encoder(x_bnk, grid=grid_bn2)
...
class PixioDecoder(nn.Module):
    ...
    def __init__(self, encoder_cfg: transformer.Config, cfg: PixioConfig):
        super().__init__()
        self.encoder_cfg = encoder_cfg
        self.cfg = cfg
        ...
        decoder_cfg = transformer.Config(
        ...
        self.blocks = nn.ModuleList([
            transformer.Block(decoder_cfg) for _ in range(cfg.decoder_depth)
        ])
